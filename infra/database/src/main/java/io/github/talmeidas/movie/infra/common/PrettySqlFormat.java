...
public class PrettySqlFormat implements MessageFormattingStrategy {
    ...
    private String formatSql(final String category, final String sql) {
        ...
        final String formatted = shouldUseDDL ? FormatStyle.DDL.getFormatter().format(sql) : FormatStyle.BASIC.getFormatter().format(sql);
        return "\nPrettySqlFormat(P6Spy SQL,Hibernate format):" + formatted;
    }
}
